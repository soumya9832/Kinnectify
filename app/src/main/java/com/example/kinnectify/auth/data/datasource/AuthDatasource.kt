package com.example.kinnectify.auth.data.datasource
import android.net.Uri
import com.example.kinnectify.auth.domain.models.CreateUserInput
import com.example.kinnectify.common.AppDispatcher
import com.example.kinnectify.common.CustomAuthException
import com.example.kinnectify.common.CustomDataException
...
class AuthDatasourceFirebase @Inject constructor(
    ...
    override suspend fun signInWithEmailAndPassword(email: String, password: String): User =
        withContext(ioDispatcher) {
            ...
            try {
                ...
                val dataSnapshot =
                    refDatabase.child(Constants.USER).child(firebaseUser!!.uid).get().await()
    ...
    private suspend fun setUserInfoOnDatabase(user: User): User = coroutineScope {
        ...
        user.token = token
        refDatabase.child(Constants.USER).child(user.id).setValue(user).await()
        user
    }
